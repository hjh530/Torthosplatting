...
def get_scales(key, cameras, images, points3d_ordered, args):
    ...
    invmonodepthmap = cv2.imread(f"{args.depths_dir}/{image_meta.name[:-n_remove]}.tiff", cv2.IMREAD_UNCHANGED)
...
if __name__ == '__main__':
    ...
    ext = f".{args.model_type}" if args.model_type else ""
    # 优先读 sparse/1（todm 输出），否则 sparse/0
    sparse_d = "1" if os.path.isdir(os.path.join(args.base_dir, "sparse", "1")) else "0"
    cam_intrinsics, images_metas, points3d = read_model(os.path.join(args.base_dir, "sparse", sparse_d), ext=ext)
    ...
    with open(f"{args.base_dir}/sparse/{sparse_d}/depth_params.json", "w") as f:
